...
class Alternatif extends Model
{
  ...
  {
    return $this->belongsTo(Aktifita::class, 'aktifitas_object_id');
  }
  ...
  {
    $results = static::with('criteria', 'aktifitas')
    ...
    foreach ($results as $result) {
      $isExists = array_search($result->aktifitas_object_id, array_column($finalRes, 'aktifitas_object_id'));
      ...
      if ($isExists !== '' && $isExists !== null && $isExists !== false) {
        ...
        $data = [
          'aktifitas_object_id' => $result->aktifitas_object_id,
          'aktifitas_name'      => $result->aktifitas->name,
