...
class PerbandinganC extends Controller
{
  public function index()
  {
    ...
    return view('pages.dashboard.kriteriaPerbandingan.index', [
  ...
  public function show(CriteriaAnalysis $criteriaAnalysis)
  {
    ...
    return view('pages.dashboard.kriteriaPerbandingan.input-value', [
  ...
  {
    $idDetail = CriteriaAnalysisDetail::where('criteria_analysis_id', $criteriaAnalysisId)
      ->whereNotIn('id', $detailIds)
      ->get();


    if ($idDetail->count()) {
      $idDetail->each(function ($value, $key) use ($criteriaAnalysisId) {
        ...
        $hasilPerbandingan = 1 / $prevComparison['comparison_value'];
        ...
        CriteriaAnalysisDetail::where([
          ...
          ->update(['comparison_result' => $hasilPerbandingan]);
      });
    }
  }

  private function _countPreventiveValue($kriteriaAId)
  {
    $kriteria = CriteriaAnalysisDetail::getSelectedCriterias($kriteriaAId);

    foreach ($kriteria as $criteria) {

      $dataDetails = CriteriaAnalysisDetail::select('criteria_id_second', 'comparison_result')
        ->where([
          'criteria_analysis_id' => $kriteriaAId,
      ...
      $FinalPrevValue = $tempValue / $kriteria->count();
